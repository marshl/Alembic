...
public class EffectExpandableListAdapter extends BaseExpandableListAdapter
{
	...
	{
		return game.effectToIngredientMap.get( game.effectList.get( _groupPosition ) ).get( _childPosition );
	}
	...
	{
		final Ingredient ingred = (Ingredient)getChild( _groupPosition, _childPosition );
		...
		label.setText( ingred.name );
		image.setImageResource( ingred.imageID );
	...
	{
		if ( game.effectList.size() <= _groupPosition )
		{
			return 0;
		}
		String str = game.effectList.get( _groupPosition );
		return game.effectToIngredientMap.get( str ).size();
	}

	public Object getGroup( int _groupPosition )
	{
		return game.effectList.get( _groupPosition );
	}

	public int getGroupCount()
	{
		return game.effectList.size();
	}
			...
			View _convertView, ViewGroup _parent ) 
	{
		...
		if ( _convertView == null )
		{
			...
			_convertView = infalInflater.inflate( R.layout.effect_group_item,
					_parent, false );
		}
		...
		int imageID = AlchemyApplication.instance.getEffectIcon( this.context, effectName, this.game.prefix );
